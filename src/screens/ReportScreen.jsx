            ...
            {(report.content.themes?.length > 0 || sentimentDist) && (
              <Section number={sn.themes} title={isKo ? "테마 & 감정 분석" : "Themes & Sentiment"} dk={dk}>
                <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                  ...
                  {report.content.themes?.length > 0 && (
                    <div>
                      <div style={{ fontSize: 10, fontWeight: 600, color: dk.dim, letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: 10 }}>{isKo ? "주제 클러스터" : "Topic Clusters"}</div>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                        {report.content.themes.map((theme, i) => (
                          <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "5px 11px", borderRadius: 20, background: dk.card2, border: `1px solid ${dk.border}`, fontSize: 12, color: dk.text }}>
                            {theme.label}
                            {theme.count && <span style={{ fontSize: 10, fontWeight: 600, color: dk.muted }}>{theme.count}</span>}
                          </span>
                        ))}
                      </div>
                    </div>
