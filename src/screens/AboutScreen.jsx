      ...
      <div style={{ background: C.navy, padding: isMobile ? "56px 24px 48px" : "80px 40px 72px", textAlign: "center" }}>
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: C.purple, letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>
            {isKo ? "Voice Survey 소개" : "About Voice Survey"}
          </div>
          ...
          <p style={{ fontSize: 16, color: "rgba(255,255,255,0.55)", lineHeight: 1.8, margin: 0 }}>
            {isKo ? <>Voice Survey는 AI 기반 음성 인터뷰 플랫폼이에요.<br />패널 모집부터 인터뷰 진행, 분석 리포트까지 — 하나로 끝나요.</> : <>Voice Survey is an AI-powered voice interview platform.<br />From panel recruitment to interview execution to analysis reports — one tool for it all.</>}
          </p>
        </div>
      </div>
      ...
      <main style={{ maxWidth: 800, margin: "0 auto", padding: isMobile ? "40px 20px 60px" : "64px 24px 80px" }}>
        ...
        <section style={{ marginBottom: 64 }}>
          ...
          <p style={{ fontSize: 15, color: C.body, lineHeight: 1.9, margin: 0 }}>
            {isKo
              ? "좋은 제품은 유저를 진짜로 이해하는 팀에서 나와요. 그런데 유저 리서치는 비용이 커요. 일정 잡고, 녹취 풀고, 패턴 찾는 데만 일주일이 걸리죠. Voice Survey는 그 과정을 10분으로 줄여요. 리서치 예산이 없는 초기 스타트업도, 글로벌 스터디를 돌리는 대기업도 똑같이 쓸 수 있어요."
