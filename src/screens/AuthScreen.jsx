...
export default function AdvertiserLoginScreen({ go, lang = "ko" }) {
  ...
  const handleSendOtp = async () => {
    if (!phone) { setAuthError("전화번호를 입력해주세요"); return; }
    const normalized = phone.startsWith("+") ? phone : `+82${phone.replace(/^0/, "")}`;
    setAuthError("");
    setLoading(true);
    const { error } = await supabase.auth.signInWithOtp({ phone: normalized });
    setLoading(false);
    if (error) { setAuthError(error.message); } else { setOtpSent(true); }
  };

  const handleVerifyOtp = async () => {
    if (!otp) { setAuthError("인증 코드를 입력해주세요"); return; }
    const normalized = phone.startsWith("+") ? phone : `+82${phone.replace(/^0/, "")}`;
    setAuthError("");
    setLoading(true);
    ...
    const afterLogin = localStorage.getItem("voica_after_login");
    if (afterLogin) { localStorage.removeItem("voica_after_login"); go(afterLogin); } else { go("dashboard"); }
  };
      ...
      <div style={{ flex: 1, display: "flex", justifyContent: "center", padding: isMobile ? (tab === "signup" ? "24px 16px 40px" : "32px 16px") : (tab === "signup" ? "32px 24px 48px" : "60px 24px") }}>
        <div style={{ width: "100%", maxWidth: 440 }}>
          ...
          <div style={{ background: C.white, borderRadius: 12, padding: "28px 28px", boxShadow: S.card }}>
            ...
            {tab === "login" && !resetMode && (
              <>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 9, marginTop: 4 }}>
                  <Btn variant="white" full size="md" style={{ background: "#FEE500", color: "#191919", border: "none" }} onClick={() => handleSocialLogin("kakao")}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="#191919"><path d="M12 3C6.477 3 2 6.477 2 10.8c0 2.7 1.636 5.08 4.113 6.513L5.2 21l5.05-2.63C10.74 18.45 11.37 18.5 12 18.5c5.523 0 10-3.477 10-7.7C22 6.477 17.523 3 12 3z"/></svg>
                    카카오로 계속하기
                  </Btn>
                  ...
                  <Btn variant="white" full size="md" style={{ background: "#f5f5f7" }} onClick={() => { setPhoneMode(true); setAuthError(""); }}>
                    📱 전화번호로 로그인
                  </Btn>
            ...
            {tab === "login" && phoneMode && (
              <>
                <div style={{ fontSize: 13, color: C.navy, marginBottom: 14 }}>
                  전화번호로 인증 코드를 받아 로그인하세요.
                </div>
                {!otpSent ? (
                  <>
                    <Input label="전화번호" type="tel" placeholder="010-1234-5678" value={phone} onChange={e => setPhone(e.target.value)} />
                    ...
                    <Btn full size="lg" style={{ marginTop: 14 }} disabled={loading || !phone} onClick={handleSendOtp}>
                      {loading ? "전송 중..." : "인증 코드 받기"}
                    </Btn>
                  </>
                ) : (
                  <>
                    <div style={{ fontSize: 12, color: C.success, marginBottom: 12, padding: "8px 12px", background: "rgba(21,190,83,0.08)", borderRadius: 6 }}>
                      {phone}로 인증 코드를 보냈어요.
                    </div>
                    <Input label="인증 코드" type="text" placeholder="6자리 코드 입력" value={otp} onChange={e => setOtp(e.target.value)} />
                    {authError && <div role="alert" style={{ marginTop: 10, padding: "8px 12px", borderRadius: 6, background: "rgba(217,48,37,0.08)", color: C.ruby, fontSize: 13 }}>{authError}</div>}
                    <Btn full size="lg" style={{ marginTop: 14 }} disabled={loading || !otp} onClick={handleVerifyOtp}>
                      {loading ? "확인 중..." : "로그인"}
                    </Btn>
                  </>
                )}
                <div style={{ textAlign: "center", marginTop: 12 }}>
                  <a href="#" onClick={e => { e.preventDefault(); setPhoneMode(false); setOtpSent(false); setOtp(""); setPhone(""); setAuthError(""); }} style={{ fontSize: 12, color: C.body, textDecoration: "none" }}>
                    ← 이메일 로그인으로 돌아가기
                  </a>
                </div>
              </>
